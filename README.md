...
| 내 폴더에 뭐가 빠졌는지 보기 | **내 폴더와 비교** — 기기의 폴더를 골라 파일 이름으로 견줍니다(경로까지 / 이름만). 저장소에만 있는 것은 골라 받고, 내 폴더에만 있는 것은 골라 올립니다(열람자는 변경 제안으로). PC 앱·안드로이드는 받은 파일을 그 폴더에 바로 넣고, 브라우저는 내려받기 폴더로 받습니다. |
